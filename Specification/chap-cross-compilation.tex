...
the \emph{target}.  When we say \emph{entirely}, we exclude the use of
the \texttt{read} function of the host environment because it can
cause some problem.  It would be possible, to use the \emph{read}
function of the host, and the difference is small.%
\footnote{The main difference that is important to bootstrapping is
  ...
  (Section 2.4.6.1).}
See \refSec{sec-reader-portability-problems} for a discussion of
...
The standard \cl{} macros with compile-time side effects are:
...
standard \cl{} function).  Our solution is thus to provide a
