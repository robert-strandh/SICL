...
\emph{tied} to $E_1$.  During phase~1, the ordinary definition of
\texttt{defclass} is used, so that the expanded form makes a call to
\texttt{ensure-class}.  We supply a special version of
\texttt{ensure-class} in phase~1.  This special version invokes the
host version of \texttt{ensure-class} with a class name that is
generated by \texttt{gensym} so that there are no clashes with
existing classes in the host.  The host version of
\texttt{ensure-class} returns an ordinary host class which is
associated with the original name in $E_1$.

When the host executes the calls to \texttt{ensure-class}, it
automatically adds accessor methods to reader functions for all these
metaobjects.  Since these reader functions were automatically imported
into $E_1$ when this environment was created, the new methods are
automatically available during bootstrapping.
