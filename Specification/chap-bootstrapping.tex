...
$R_1$.  In particular, we need following functions from the host:

\begin{itemize}
\item \texttt{generic-function-method-class} is needed by the macro
  \texttt{defmethod} in order to instantiate a method of the right
  class to be added to a host generic function.
\item \texttt{class-prototype} is also needed by the macro
  \texttt{defmethod} in order to create instances to pass to
  \texttt{make-method-lambda} for the purpose of dispatch.
\end{itemize}
