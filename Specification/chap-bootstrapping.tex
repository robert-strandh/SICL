...
Phase 1 is characterized by the fact that the MOP classes are host
classes and the MOP generic functions are host generic functions.  We
use \texttt{host:defclass} to create a hierarchy of host classes
corresponding to the hierarchy of MOP classes.  The names of those
host classes will be \texttt{target:standard-object},
...
\texttt{T} does not supply any slots anyway.  In order to be able to
use the file of MOP classes unmodified, we define a version if
...
to \texttt{nil} instead.  These host classes created here have the
...
\texttt{host:standard-class} as metaclass.  The class finalization
protocol of the host will make sure that instances of these host
classes have the effective slots determined by class inheritance.  In
other words, each of the classes define slots with the right names,
the right initargs, etc.
...
The purpose of this phase is to create bridge classes and bridge
generic functions corresponding to MOP classes and MOP generic
functions.  For that purpose, we define the specified macros
\texttt{defclass}, \texttt{defgeneric}, and \texttt{defmethod}.  These
macros expand to calls to corresponding \texttt{ensure-...} functions
of phase 1.  These macros need some support code that is also defined
here, such as the canonicalization functions for \texttt{defclass} and
\texttt{make-method-lambda} for \texttt{defmethod}.
