...
\texttt{ensure-class}.  We avoid this problem by \emph{importing}
\texttt{make-instance} from $E_0$, so that the class names given to
\texttt{make-instance} are looked up in $E_0$ rather than in $E_1$.
This way, modifications to the set of classes available in $E_1$ will
not interfere with the classes that \texttt{make-instance} is asked to
instantiate.
