...
Conceptually, upon entry to a function, the arguments are immediately
saved to a vector that becomes the first entry of the \emph{lexical
  environment level} created by this function.  The remaining entries
of that lexical environment level are the lexical variables used by
this function, including temporaries.  The arguments in the stack
frame are then removed so that the stack frame is empty, aside from
the four words that are always present.
\refFig{fig-x86-32-conceptual-argument-parsing} illustrates this
conceptual situation.

\begin{figure}
\begin{center}
\inputfig{fig-x86-32-conceptual-argument-parsing.pdf_t}
\end{center}
\caption{\label{fig-x86-32-conceptual-argument-parsing}
Conceptual situation for argument parsing.}
\end{figure}

Occasionally (but not often), the situation in
\refFig{fig-x86-32-conceptual-argument-parsing} is what really
happens.  For instance, if the first parameter to a function is a
\emph{special variable}, then, in \sysname{}, we implement the dynamic
binding of that special variable as a function call, passing the
symbol that names the variable, the value of the binding, and a
\emph{closure} that is responsible for the remaining computation. 
In terms of argument parsing, the fact that the remaining
argument-parsing process is accomplished by that closure, i.e. by a
different function invocation, requires the remaining arguments to be
accessible through the lexical environment.  

What makes it necessary to store arguments in the lexical environment
is the fact that the argument-parsing process is \emph{closed over}.
This situation can be detected statically, because the lexical
variable holding the argument vector is then closed over.  While it
would be very inefficient to parse arguments this way, we believe that
this situation is sufficiently infrequent that no optimization is
required. 

When the argument-parsing process itself is not closed over, all the
arguments can remain on the stack.  In the most general case, lexical
and temporary variables will still have to be allocated as a level in
the lexical runtime environment.  In this case, the arguments will
remain in the stack frame until all lexical variables have been
initialized, and then they are removed.  
...
variables and temporaries that are not closed over. 
