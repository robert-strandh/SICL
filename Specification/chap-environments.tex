...
\item The item is present but not marked as \emph{created}.  This
  ...
  variable.  If the type check does not pass, then an error is
  signaled.  Whether or not the \texttt{defvar} has an \emph{initial
    value} form, the entry is marked as \emph{created}.
...
for a variable, the mapping is searched for an entry.  If no entry is
found, one is created, and its type is set accordingly.%
\fixme{Is it really correct to create the variable just because its
  type is declared?  If not, is it OK to just put a flag in the entry
  indicating whether it has been created or not? 
  Also, explain what happens when the entry exists.}
