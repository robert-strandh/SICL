...
sequentially from $0$ as a class is initialized or reinitialized.
When a class is reinitialized, its old unique number is never reused,
...
effective method to invoke.  The class numbers become constants inside
the compiled code of the discriminating function, making comparison
fast.  Each argument class is tested in the order determined by the
\emph{argument precedence order} of the generic function.  For each
argument, the set of possible effective methods is filtered by a
binary search, eliminating around half of the possibilities in each
comparison.
