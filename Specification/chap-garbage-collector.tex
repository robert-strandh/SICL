...
\texttt{cons} cell or a \emph{general instances} represented as a
...
an ordinary memory allocator, for example the one created by Doug
Lea.%
...
Doug Lea's allocator is adapted to our specific situation in a few
ways.  \refFig{fig-chunks} show how available address space is divided
into \emph{chunks}.%
\footnote{This figure is adapted from http://g.oswego.edu/dl/html/malloc.html}
\begin{figure}
\begin{center}
\inputfig{fig-chunks.pdf_t}
\end{center}
...
\item Requesting roots.  In this phase, the global collector indicates
  ...
  blocked application thread, a thread is spawned to do the work on
  its behalf.
...
has a allocated threads that are blocked or no threads allocated
...
addition, the gray bitmap have a multi-level \emph{index}, making it
...
cleared.  If this operation results in the 64-bit word that this bit
is contained in having all bits cleared, then bit position in the
...
th cache).  The cache could for instance be a vector with around a
...
dyads to the beginning of the nursery heap and to slide racks so the
