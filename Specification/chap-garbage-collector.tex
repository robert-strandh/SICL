...
representation where every heap allocated object has a two-word header
object.  If it is a \texttt{cons} cell, then that is the entire
object.  For other heap-allocated objects (called \emph{general
  instances}, the first of the two words is a tagged pointer to the
class object, and the second of the two words is a pointer to the
\emph{rack} using a special tag for racks.  In this chapter, we
describe the consequences of this representation to the garbage
collector.
...
to be an immediate object (fixnum, character, etc) or when it is an
...
headers to the beginning of the nursery heap and to slide rack so the
...
two \emph{break tables} between the new locations of the header free
...
$d_i$ added to it.  One break table is built for headers and another
...
Once break table are built, the stack is scanned to find roots, and it
...
to a header or to a rack, and consult the corresponding break table to
determine a value to be added to the current one.

Next, headers in the nursery are traversed.  For a \texttt{cons} cell,
