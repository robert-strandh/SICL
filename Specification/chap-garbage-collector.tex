...
\item Determining roots.  In this phase, the collector
  indicates to each running application thread that it needs to
  transmit its part of the global root set to the collector.
  For each blocked application thread, another thread (newly spawned
  or taken from a thread pool) does the work on its behalf.
\item Waiting for roots.  In this phase, the collector waits
...
the collector can not run until every application thread has
...
We think that in cases like that, one of the threads of the
...
that thread and the collector threads.  The variable has 4 bit
...
\item \texttt{gc-requested} meaning that the collector has
  requested that the application thread run the garbage collector.
  This bit is set by the collector and cleared by the
  application thread.
\item \texttt{gc-in-progress} meaning that the collector is
  running the nursery garbage collector on behalf of the thread.  This
  bit is both set and cleared by the collector.
\item \texttt{application-waiting} meaning that the application is
  waiting for the collector to finish the garbage collection on
...
\item \texttt{global-gc-may-proceed}.  This semaphore is initialized to
  $0$.  When the collector requests that an application thread
...
\item \texttt{application-may-execute}.  This semaphore is initialized
  ...
  blocked, it checks whether the collector is currently
  executing on behalf of this application thread.  If so, the
  application thread waits on this semaphore.  When the
...
\section{Running application thread}
...
indicating to the collector that it has finished, and finally
...
\section{Application thread about to block}
...
operation,  it must inform the collector that it might be
unable to run the local garbage collector itself, and that the
...
collection, it signals the \texttt{global-gc-may-proceed} semaphore
indicating to the collector that it has finished, and then
...
If the CAS operation succeeds, it means that the collector has
...
\section{Application thread waking up after block}

When the application thread wakes up after having been blocked, there
is a possibility that the garbage collector is in the process
...
operation.  If the CAS operation succeeds, this means that the
...
If the CAS operation fails, then that means that the collector
...
operation succeeds, it means that the collector is still
running the nursery collector on behalf of the application thread, and
that the collector has been informed that, when the nursery
