...
\item The \emph{file compiler}.  This compiler is invoked by
  ...
  file containing object code (a so-called \emph{fasl} file).  Since
  \sysname{} \emph{fasl} files are merely textual versions of the
  abstract syntax tree produced by the \texttt{cst-to-ast} phase of
  \cleavir{} as described in \refChap{chap-compiled-files}, only that
  phase is involved in file compilation.
