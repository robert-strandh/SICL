...
\texttt{sicl-primitives}.  The following primitives are defined by
\sysname{}:

\begin{itemize}
\item \texttt{car}.  Return the \texttt{car} slot of an object known
  to be a \texttt{cons} cell.
\item \texttt{rplaca}.  Store an arbitrary object in the \texttt{car}
  slot an object known to be a \texttt{cons} cell.
\item \texttt{cdr}.  Return the \texttt{cdr} slot of an object known
  to be a \texttt{cons} cell.
