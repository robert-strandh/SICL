...
\item If either:
  ...
  \item $b$ is assigned only to a stack location, $I$ permits a memory
    location as its second operand, and the next use of $b$ is far in
    the future,
  \end{legal}
  then no further processing is done for $b$.
\item Otherwise, a register is allocated for $b$ as a second operand,
  as described below.
\end{legal}

\begin{legal}
\item $a$ is an immediate operand.  $b$ is on the stack.
  \begin{legal}
  \item $I$ is commutative. Swap the inputs and handle the resulting
    case.
  \item $I$ is not commutative.  Allocate a register for $a$, say
    $r$.  Insert an assignment instruction that assigns $a$ to $r$.
    Handle the resulting case.
  \end{legal}
\item $a$ is an immediate operand.  $b$ is in a register, say $s$.
  \begin{legal}
  \item $I$ is commutative. Swap the inputs and handle the resulting
    case.
  \item $I$ is not commutative.  Allocate a register for $a$, say
    ...
    Handle the resulting case.
  \end{legal}
\item $b$ is an immediate operand.  $a$ is on the stack.
  \begin{legal}
  \item Next time $c$ is needed, it is needed in a specific register,
    say $t$.
    \begin{legal}
    \item $t$ is not assigned to any variable.  Insert and instruction
      to load $a$ into $t$.  Assign $t$ to $c$.
    \item $t$ is already assigned to some variable $v$.
      \begin{legal}
      \item $v$ is needed sooner than $c$.  Allocate a new register,
        say $u$, and assign it to $c$.  Insert an instruction for
        loading $a$ into $u$.
      \item $c$ is needed sooner than $v$.
        \begin{legal}
        \item Next time $v$ is needed, it is needed in some specific
          register $w$.
          \begin{legal}
          \item $w = t$.  Allocate a new register $u$ and assign it to
            $v$.  Insert an instruction assigning $t$ to $u$.  Insert
            an instruction for loading $a$ into $t$.  Assign $t$ to
            $v$.
          \item $w \not = t$.
            \begin{legal}
            \item $w$ is not currently assigned to any variable.
              Insert an instruction for copying $t$ to $w$.  Assign
              $w$ to $v$.  Insert an instruction for loading $a$ into
              $t$.  Reassign $t$ to $c$.
            \item $w$ is currently assigned to a variable $p$.
              \begin{legal}
              \item $p$ is also present on the stack.  Insert an
                instruction copying $t$ to $w$.  Reassign $w$ to $v$.
                Insert an instruction for loading $a$ into $t$.
                Assign $t$ to $c$.
              \item $p$ is not present on the stack.  Insert an
                instruction for spilling $w$ to the stack. Insert an
                instruction copying $t$ to $w$.  Reassign $w$ to $v$.
                Insert an instruction for loading $a$ into $t$.
                Assign $t$ to $c$.
              \end{legal}
            \end{legal}
          \end{legal}
        \item Next time $v$ is needed, it can be in any register.
          Allocate a new register $u$.  Insert an instruction copying
          $t$ to $u$.  Assign $u$ to $v$.  Insert an instruction for
          loading $a$ into $t$.  Reassign $t$ to $c$.
        \end{legal}
      \end{legal}
    \end{legal}
  \item Next time $c$ is needed, it can be in any register.  Allocate
    a new register $u$.  Assign it to $c$.  Insert an instruction for
    loading $a$ into $u$.
  \end{legal}
\item $b$ is an immediate operand.  $a$ is in a register, say $r$.
\item $a$ and $b$ are both on the stack.
\item $a$ is in a register, say $r$.  $b$ is on the stack.
\item $b$ is in a register, say $s$.  $a$ is on the stack.
\item $a$ and $b$ are both in register, say $r$ and $s$ respectively.
  \begin{legal}
        ...
        \item $c$ is needed soon after $I$.
          ...
          \item $c$ is needed in a specific register, say $t$.
            ...
            \item $t$ is currently assigned to some variable $v$.
              ...
              \item $c$ is needed sooner than $v$.  Reassign $t$ to
                $c$.  Insert and instruction before $I$ that copies
                $r$ into $t$.
              \end{legal}
            \end{legal}
