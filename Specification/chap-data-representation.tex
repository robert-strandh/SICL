...
\item The executable native instructions in the form of a vector of
  unsigned bytes.
\item A table mapping a value $P$ of the program counter to either
  another value $N$ of the program counter, or to a list of such
  values.  Both values correspond either to the beginning or to the
  end of a form to be evaluated.  This table is used to support the
  \texttt{step in} command of the debugger.  A list of values is
  needed when a branch in the code exists before the next forms to be
  evaluated.
\item A table mapping a value $P$ of the program counter to another
  value $N$ of the program counter.  Both values correspond either to
  the beginning or to the end of a form to be evaluated.  This table
  is used to support the \texttt{step out} command of the debugger.
\item A table mapping a value $P$ of the program counter to either
  another value $N$ of the program counter, or to a list of such
  values.  Both values correspond either to the beginning or to the
  end of a form to be evaluated.  This table is used to support the
  \texttt{step over} command of the debugger.  A list of values is
  needed when a branch in the code exists before the next forms to be
  evaluated.
\item A table mapping a value of the program counter to a
  \emph{register map}.  This information is used by the garbage
  collector to determine which registers contain \commonlisp{} objects
  that should be traced.  Only values of the program counter
  corresponding to \emph{safe points} are present in the table.  A
  safe point is either the value of the program counter immediately
  after a \texttt{call} instruction, or immediately before a branch
  instruction jumping to a lower address.
