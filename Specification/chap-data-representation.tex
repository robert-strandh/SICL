...
\item A table mapping a value of the program counter to a
  \emph{register map}.  This information is used by the garbage
  collector to determine which registers may contain \commonlisp{}
  objects that should be traced.  Only values of the program counter
  corresponding to program points immediately after a \texttt{call}
  instruction are present in this table.  Only \emph{callee-saved}
  registers are represented.  Other registers will have been saved to
  the stack when this information is required by the garbage
  collector.  The compiler may omit registers that are known to
  contain immediate \commonlisp{} objects that the garbage collector
  does not have to trace.
\item A table mapping a value of the program counter to a \emph{stack
  frame map}.  This information is used by the garbage collector to
  determine which stack locations may contain \commonlisp{} objects
  that should be traced.  Only values of the program counter
  corresponding to program points immediately after a \texttt{call}
  instruction are present in this table.  The compiler may omit
  registers that are known to contain immediate \commonlisp{} objects
  that the garbage collector does not have to trace.
