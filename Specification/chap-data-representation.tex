...
\item A table mapping a value $P$ of the program counter to another
  ...
  the beginning or to the end of a form to be evaluated.  The table is
  used by the \emph{next} command of the \emph{stepping function} of a
  debugger to find the next place to stop, given the current stopping
  point.  If $P$ corresponds to the beginning of a form, then $N$
  corresponds to the end of the same form.  If $P$ corresponds to the
  end of a form, then $N$ corresponds either to the beginning of the
  next form to be evaluated, or to the end of that form, depending on
  the relationship between those two forms.
\item A table mapping a value of the program counter to a
  \emph{register map}.  This information is used by the garbage
  collector to determine which registers contain \commonlisp{} object
