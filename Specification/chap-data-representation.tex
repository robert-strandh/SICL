...
\item A hash table mapping a value of the program counter to a
  ...
  locations contain \commonlisp{} objects that should be traced.  Only
  values of the program counter corresponding to \emph{safe program
    points} are present in this table.  Recall that a \emph{safe
    program point} is either the address of the instruction
  immediately after a \texttt{call} instruction or the address of a
  control-transfer instruction with a target having a smaller address.
  Stack locations containing data owned by the current function
  invocation are present in the table.  If a datum owned by the
  current function invocation is stored in a callee-saves register,
  the stack location where the old value of that register is stored is
  indicated instead.  The compiler may omit stack locations that are
...
\item A table mapping a value of the program counter to a
  \emph{callee-saves map}.  This map is indexed by a callee-saved
  ...
  corresponding entry contains some default value ($0$) indicating
  this fact.  For a typical backend such as x86-64, the entries in the
  table represent the registers \texttt{RBX} (index 0), \texttt{R12}
  (index 1), \texttt{R13} (index 2), \texttt{R14} (index 3), and
  \texttt{R15} (index 4) as described in
