...
\begin{itemize}
\item A list of stack locations that may contain objects that the
...
\item A hash table mapping a value of the program counter to a
  ...
  invocation are present in the table.  The compiler may omit stack
  locations that are known to contain immediate \commonlisp{} objects
  that the garbage collector does not have to trace.  For a typical
  backend, index $i$ of the bitmap represents the stack location at
  address $b-i$ where $b$ is the value of the base pointer.
