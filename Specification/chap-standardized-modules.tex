...
\texttt{Code/Arithmetic}.  There are several \asdf{} system
definitions for various aspects of this code.  The package for this
code is \texttt{sicl-arithmetic}.

The code contains definitions of the functions functions in the
\texttt{numbers} dictionary of the standard.  Each function that takes
an arbitrary number of arguments calls a version that takes exactly
two arguments and this version is generic with methods specialized to
the different classes of the arguments.  Functions that take a fixed
number of arguments are already generic.

Methods that cannot be implemented using portable code call the
function \texttt{sicl-primop:primop}.

During bootstrapping time, methods on \texttt{sicl-primop:primop} for
integer arithmetic are implemented using host code.  Methods for
floating-point arithmetic use simulated floating-point arithmetic with
functions supplied by the system \texttt{buoy-simulate}.

Run-time definitions of the elementary functions such as \texttt{sin},
\texttt{exp}, etc. will be supplied by the \texttt{Buoy} library.
