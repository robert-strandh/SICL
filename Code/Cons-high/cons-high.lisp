...
(defun intersection-identity-eql (list1 list2)
  (let ((result '()))
    (with-proper-list-rests (rest1 list1 intersection)
      (let ((element1 (car rest1)))
	(when (with-proper-list-rests (rest2 list2 intersection)
		(when (eql element1 (car rest2))
		  (return t)))
...
(defun intersection-key-eql (list1 list2 key)
  (let ((result '()))
    (with-proper-list-rests (rest1 list1 intersection)
      (let ((element1 (car rest1)))
	(when (with-proper-list-rests (rest2 list2 intersection)
		(when (eql (funcall key element1) (funcall key (car rest2)))
		  (return t)))
	  (push element1 result))))
    result))
...
(defun intersection-key-eq (list1 list2 key)
  (let ((result '()))
    (with-proper-list-rests (rest1 list1 intersection)
      (let ((element1 (car rest1)))
	(when (with-proper-list-rests (rest2 list2 intersection)
		(when (eq (funcall key element1) (funcall key (car rest2)))
		  (return t)))
	  (push element1 result))))
    result))
...
(defun intersection-identity-test (list1 list2 test)
  (let ((result '()))
    (with-proper-list-rests (rest1 list1 intersection)
      (let ((element1 (car rest1)))
	(when (with-proper-list-rests (rest2 list2 intersection)
		(when (funcall test element1 (car rest2))
		  (return t)))
	  (push element1 result))))
    result))
...
(defun intersection-key-test (list1 list2 key test)
  (let ((result '()))
    (with-proper-list-rests (rest1 list1 intersection)
      (let ((element1 (car rest1)))
	(when (with-proper-list-rests (rest2 list2 intersection)
		(when (funcall test (funcall key element1) (funcall key (car rest2)))
		  (return t)))
	  (push element1 result))))
    result))
...
(defun intersection-identity-test-not (list1 list2 test-not)
  (let ((result '()))
    (with-proper-list-rests (rest1 list1 intersection)
      (let ((element1 (car rest1)))
	(when (with-proper-list-rests (rest2 list2 intersection)
		(unless (funcall test-not element1 (car rest2))
		  (return t)))
	  (push element1 result))))
    result))
...
(defun intersection-key-test-not (list1 list2 key test-not)
  (let ((result '()))
    (with-proper-list-rests (rest1 list1 intersection)
      (let ((element1 (car rest1)))
	(when (with-proper-list-rests (rest2 list2 intersection)
		(unless (funcall test-not (funcall key element1) (funcall key (car rest2)))
		  (return t)))
	  (push element1 result))))
    result))
...
(defun intersection-identity-eq-hash (list1 list2)
  (let ((table (make-hash-table :test #'eq))
	(result '()))
    (with-proper-list-rests (rest list2 intersection)
      (let ((element (car rest)))
	(setf (gethash element table) t)))
    (with-proper-list-rests (rest list1 intersection)
      (let ((element (car rest)))
