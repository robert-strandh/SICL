...
(defmacro for-each-relevant-cons ((cons-var list start end from-end) &body body)
  (let ((fun-var (gensym))
	(list-var (gensym))
	(rest-var (gensym))
	(length-var (gensym))
	(iter-var (gensym)))
    `(let (,cons-var ,length-var (,list-var ,list))
       (if ,from-end
	   (progn (when (null ,end)
		    (setf ,end (length ,list-var)))
		  (if (null ,start)
		      (setf ,start 0)
		      (setf ,list-var (nthcdr ,start ,list-var)))
		  (setf ,length-var (- ,end ,start))
		  (labels ((,fun-var (,rest-var ,length-var)
