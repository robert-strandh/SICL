...
(defun |nsubstitute-if-not seq-type=list from-end=nil end=nil count=other key=identity|
    (newitem predicate list start count)
  (let ((remaining list)
	(start-bis start))
    ;; skip a prefix indicated by start
...
(defun |nsubstitute-if-not seq-type=list from-end=nil end=nil count=other key=other|
    (newitem predicate list start count key)
  (let ((remaining list)
	(start-bis start))
    ;; skip a prefix indicated by start
...
(defun |nsubstitute-if-not seq-type=list from-end=nil end=other count=other key=identity|
    (newitem predicate list start end count)
  (let ((remaining list)
	(start-bis start)
	(end-start (- end start)))
...
(defun |nsubstitute-if-not seq-type=list from-end=nil end=other count=other key=other|
    (newitem predicate list start end count key)
  (let ((remaining list)
	(start-bis start)
	(end-start (- end start)))
...
(defun |nsubstitute-if-not seq-type=list from-end=t end=nil count=other key=identity|
    (newitem predicate list start count)
  (let ((remaining list)
	(start-bis start))
    ;; skip a prefix indicated by start
...
(defun |nsubstitute-if-not seq-type=list from-end=t end=nil count=other key=other|
    (newitem predicate list start count key)
  (let ((remaining list)
	(start-bis start))
    ;; skip a prefix indicated by start
...
(defun |nsubstitute-if-not seq-type=list from-end=t end=other count=other key=identity|
    (newitem predicate list start end count)
  (let ((remaining list)
	(start-bis start)
	(end-start (- end start)))
...
(defun |nsubstitute-if-not seq-type=list from-end=t end=other count=other key=other|
    (newitem predicate list start end count key)
  (let ((remaining list)
	(start-bis start)
	(end-start (- end start)))
...
(defun |nsubstitute-if-not seq-type=general-vector from-end=nil count=other key=identity|
    (newitem predicate vector start end count)
  (loop for i from start below end
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=general-vector from-end=nil count=other key=other|
    (newitem predicate vector start end count key)
  (loop for i from start below end
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=general-vector from-end=t count=other key=identity|
    (newitem predicate vector start end count)
  (loop for i downfrom (1- end) to start
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=general-vector from-end=t count=other key=other|
    (newitem predicate vector start end count key)
  (loop for i downfrom (1- end) to start
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=simple-vector from-end=nil count=other key=identity|
    (newitem predicate vector start end count)
  (loop for i from start below end
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=simple-vector from-end=nil count=other key=other|
    (newitem predicate vector start end count key)
  (loop for i from start below end
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=simple-vector from-end=t count=other key=identity|
    (newitem predicate vector start end count)
  (loop for i downfrom (1- end) to start
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=simple-vector from-end=t count=other key=other|
    (newitem predicate vector start end count key)
  (loop for i downfrom (1- end) to start
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=simple-string from-end=nil count=other key=identity|
    (newitem predicate vector start end count)
  (loop for i from start below end
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=simple-string from-end=nil count=other key=other|
    (newitem predicate vector start end count key)
  (loop for i from start below end
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=simple-string from-end=t count=other key=identity|
    (newitem predicate vector start end count)
  (loop for i downfrom (1- end) to start
	while (plusp count)
	do (decf count)
...
(defun |nsubstitute-if-not seq-type=simple-string from-end=t count=other key=other|
    (newitem predicate vector start end count key)
  (loop for i downfrom (1- end) to start
	while (plusp count)
	do (decf count)
...
(defun nsubstitute-if-not
  ...
  (cond ((listp sequence)
	 (if from-end
		 ...
		 (if count
		     (if key
			 (|nsubstitute-if-not seq-type=list from-end=t end=other count=other key=other|
			  newitem predicate sequence start end count key)
			 (|nsubstitute-if-not seq-type=list from-end=t end=other count=other key=identity|
			  newitem predicate sequence start end count))
		     (if key
		 ...
		 (if count
		     (if key
			 (|nsubstitute-if-not seq-type=list from-end=t end=nil count=other key=other|
			  newitem predicate sequence start count key)
			 (|nsubstitute-if-not seq-type=list from-end=t end=nil count=other key=identity|
			  newitem predicate sequence start count))
		     (if key
		 ...
		 (if count
		     (if key
			 (|nsubstitute-if-not seq-type=list from-end=nil end=other count=other key=other|
			  newitem predicate sequence start end count key)
			 (|nsubstitute-if-not seq-type=list from-end=nil end=other count=other key=identity|
			  newitem predicate sequence start end count))
		     (if key
			 (|nsubstitute-if-not seq-type=list end=other count=nil key=other|
			  newitem predicate sequence start end key)
		 ...
		 (if count
		     (if key
			 (|nsubstitute-if-not seq-type=list from-end=nil end=nil count=other key=other|
			  newitem predicate sequence start count key)
			 (|nsubstitute-if-not seq-type=list from-end=nil end=nil count=other key=identity|
			  newitem predicate sequence start count))
		     (if key
			 (|nsubstitute-if-not seq-type=list end=nil count=nil key=other|
			  newitem predicate sequence start key)
	...
	((simple-string-p sequence)
	 ...
	 (if from-end
	     (if count
		 (if key
		     (|nsubstitute-if-not seq-type=simple-string from-end=t count=other key=other|
		      newitem predicate sequence start end count key)
		     (|nsubstitute-if-not seq-type=simple-string from-end=t count=other key=identity|
		      newitem predicate sequence start end count))
		 (if key
	     ...
	     (if count
		 (if key
		     (|nsubstitute-if-not seq-type=simple-string from-end=nil count=other key=other|
		      newitem predicate sequence start end count key)
		     (|nsubstitute-if-not seq-type=simple-string from-end=nil count=other key=identity|
		      newitem predicate sequence start end count))
		 (if key
		     (|nsubstitute-if-not seq-type=simple-string count=nil key=other|
		      newitem predicate sequence start end key)
	...
	((simple-vector-p sequence)
	 ...
	 (if from-end
	     (if count
		 (if key
		     (|nsubstitute-if-not seq-type=simple-vector from-end=t count=other key=other|
		      newitem predicate sequence start end count key)
		     (|nsubstitute-if-not seq-type=simple-vector from-end=t count=other key=identity|
		      newitem predicate sequence start end count))
		 (if key
	     ...
	     (if count
		 (if key
		     (|nsubstitute-if-not seq-type=simple-vector from-end=nil count=other key=other|
		      newitem predicate sequence start end count key)
		     (|nsubstitute-if-not seq-type=simple-vector from-end=nil count=other key=identity|
		      newitem predicate sequence start end count))
		 (if key
		     (|nsubstitute-if-not seq-type=simple-vector count=nil key=other|
		      newitem predicate sequence start end key)
	...
	(t
	 ...
	 (if from-end
	     (if count
		 (if key
		     (|nsubstitute-if-not seq-type=general-vector from-end=t count=other key=other|
		      newitem predicate sequence start end count key)
		     (|nsubstitute-if-not seq-type=general-vector from-end=t count=other key=identity|
		      newitem predicate sequence start end count))
		 (if key
	     ...
	     (if count
		 (if key
		     (|nsubstitute-if-not seq-type=general-vector from-end=nil count=other key=other|
		      newitem predicate sequence start end count key)
		     (|nsubstitute-if-not seq-type=general-vector from-end=nil count=other key=identity|
		      newitem predicate sequence start end count))
		 (if key
		     (|nsubstitute-if-not seq-type=general-vector count=nil key=other|
		      newitem predicate sequence start end key)
