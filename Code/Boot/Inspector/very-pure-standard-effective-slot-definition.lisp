...
(defmethod clouseau:inspect-object-using-state
    ((object t)
     (state  inspected-very-pure-standard-effective-slot-definition)
     (style  (eql :collapsed))
     (stream t))
  (clim:with-drawing-options (stream :ink clim:+gold+)
    (format stream
            "Very purestandard effective slot definition")))
...
(defmethod clouseau:inspect-object-using-state
    ((object t)
     ...
     (stream t))
  (clim:with-drawing-options (stream :ink clim:+gold+)
    (format stream "Very purestandard effective slot definition")))
