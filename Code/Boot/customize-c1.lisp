...
;;; classes, we need to define some methods on host generic functions,
;;; specialized to classes that were created in phase 1.  The methods
;;; are auxiliary methods on the generic functions
;;; INITIALIZE-INSTANCE, REINITIALIZE-INSTANCE, and SHARED-INITIALIZE,
;;; and the purpose is to implement what the AMOP requires for
;;; classed, generic functions, and slot definitions.  We basically
;;; want to evaluate host DEFMETHOD forms that are loaded from files,
;;; except that the names of the specializer classes are wrong for the
;;; host environment.  When we created those specializer classes in
;;; phase 1, we gave them GENSYMed names so that they would not clash
;;; with existing host classes.
;;;
