...
(defun phase3 ()
  (let ((r1 *phase1-mop-class-env*)
    ...
    (setf (sicl-genv:fdefinition 'sicl-clos::compute-effective-method r2)
          #'identity)
    (setf (sicl-genv:fdefinition 'sicl-clos::no-applicable-method r2)
          (lambda (&rest arguments)
            (error "No applicable method ~s" arguments)))
    ...
    (setf (sicl-genv:fdefinition '(setf sicl-clos::standard-instance-access) r3)
          (lambda (&rest arguments)
            (error "standard-instance-access ~s" arguments)))
