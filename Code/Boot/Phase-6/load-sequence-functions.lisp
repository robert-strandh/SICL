...
(defun load-sequence-functions (e5)
  ...
  (load-source-file "Array/types.lisp" e5)
  ...
  (funcall (env:fdefinition (env:client e5) e5 'sicl-clos:remove-direct-subclass)
           (env:find-class (env:client e5) e5 'string)
           (env:find-class (env:client e5) e5 'sicl-boot::host-string)))
