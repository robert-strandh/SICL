(cl:in-package #:sicl-variant-extrinsic)
...
(defmethod cleavir-cst-to-ast::convert-global-function-reference 
    (cst
     ...
     (env sicl-extrinsic-environment:environment)
     (system sicl))
...
(defun call-with-variable-bound (variable-name value thunk)
  (setf sicl-extrinsic-environment:*dynamic-environment*
	(cons (make-instance 'variable-binding
		:symbol variable-name
		:value value)
	      sicl-extrinsic-environment:*dynamic-environment*))
  (funcall thunk))
...
(defmethod cleavir-env:function-info :around
    ((env sicl-extrinsic-environment:environment) function-name)
