(in-package #:sicl-compiler-ssa-form)

;;; Given the dominance frontiers as computed by the function
;;; SICL-COMPILE-DOMINANCE:DOMINANCE-FRONTIERS and set S of nodes,
;;; return a set PHI of nodes corresponding to the join ponts where
;;; phi functions should be inserted for a variable that is defined in
;;; the nodes in S.
;;;
...
(defun phi-function-nodes (dominance-frontiers nodes)
  (let ((result '())
    ...
    (loop until (null worklist)
	  for x = (pop worklist)
	  do (loop for y in (sicl-compiler-dominance:dominance-frontier
