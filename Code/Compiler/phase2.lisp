...
;;;
;;; 2. TYPES, which indicates what types the RESULTS should be.  If
;;; RESULTS is a list, then TYPES is a list of the same length.  If
;;; RESULTS is T, then TYPE can be a list of any length. 
;;;
;;; 3. SUCCESSORS, which is a proper list containing one or two
...
;;;
;;; 4. FALSE-REQUIRED-P, which is a Boolean value indicating whether a
...
(defun context (results types successors &optional (false-required-p t))
...
;;; Compile a CALL-AST.
;;;
;;; If the CALL-AST is compiled in a context where all the values are
;;; needed (i.e. with RESULTS equal to T), then generate a
...
;;; use one or two successors.  If it has two successors, tests the
;;; first value received and selects a successor based on whether that
;;; value is NIL or something else.
...
(defun compile-arguments (arguments temps successor)
  (loop with succ = successor
	...
	do (unless (or (typep temp 'sicl-mir:immediate-input)
	     ...
	     (setf succ (compile-ast arg (context `(,temp) '(t) `(,succ)))))
	finally (return succ)))
