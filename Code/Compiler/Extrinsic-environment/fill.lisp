...
(defun fill-environment (environment system)
  ...
  (flet ((load-file (file-name)
    ...
    (load-file "../../Evaluation-and-compilation/macro-function.lisp")
    ...
    ;; anyway.
    ;;  (load-file "../../Iteration/do-dostar-defmacro.lisp")
    ;; Load a file containing the definition of the macro
    ;; WITH-OPEN-STREAM.
    ;;  (load-file "../../Stream/with-open-stream-defmacro.lisp")
    ;; Load a file containing the definition of the macro WITH-SLOTS.
    (load-file "../../CLOS/with-slots-defmacro.lisp")
