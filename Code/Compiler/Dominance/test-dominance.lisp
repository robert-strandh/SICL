...
(defun slow-dominance-frontiers (start-node successor-fun)
  (let ((predecessor-fun (sicl-compiler-utilities:predecessor-function
			  start-node successor-fun))
	(dominance-tree (sicl-compiler-dominance:dominance-tree
			 start-node successor-fun))
	(preorder (sicl-compiler-utilities:depth-first-search-preorder
		   start-node successor-fun))
	(result (make-hash-table :test #'eq)))
    (flet ((predecessors (node)
	     (funcall predecessor-fun node))
	   (dominates (n1 n2)
	     (member n1 (sicl-compiler-dominance:dominators
			 dominance-tree n2)))
	   (strictly-dominates (n1 n2)
	     (member n1 (sicl-compiler-dominance:strict-dominators
			 dominance-tree n2))))	     
...
(defun test-dominance-frontiers-one-flow-chart (start-node successor-fun)
  (flet ((same-set-p (s1 s2)
    ...
    (let ((f1 (sicl-compiler-dominance:dominance-frontiers
	       start-node successor-fun))
  ...
  (catch 'discrepancy
    (loop repeat n
	  do (test-dominance-frontiers-one-flow-chart
	      ...
	      #'sicl-compiler-test-utilities:successors))))
