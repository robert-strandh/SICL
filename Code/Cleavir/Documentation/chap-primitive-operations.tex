...
\emph{test-form} in the special form \texttt{if}.  It should not be
used directly in application code.  Instead, its main purpose is for
defining the code for that \commonlisp{} function.  Typically the
...
This primitive operation should not be used directly in application
code.  The implementation may have a compiler macro on the function
...
This primitive operation has the same semantics as the \commonlisp{}
function \texttt{consp}.  It should not be used directly in
application code.  Instead, its main purpose is for defining the code
