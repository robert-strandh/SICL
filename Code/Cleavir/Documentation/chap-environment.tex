...
\Defgeneric {add-lexical-variable} {environment name}

This generic function is called by the compiler in order to add
\textit{name} as a lexical variable to \textit{environment}.  The
function returns a new environment augmented with the new
information.  The original environment is not affected.

\subsection{Adding special variables}

\Defgeneric {add-special-variable} {environment name}

This generic function is called by the compiler in order to add
\textit{name} as a special variable to \textit{environment}.  The
function returns a new environment augmented with the new
information.  The original environment is not affected.

\subsection{Adding local symbol macros}

\Defgeneric {add-local-symbol-macro} {environment name expansion}

This generic function is called by the compiler in order to add a
local symbol macro to \textit{environment}.  The parameter
\textit{name} is the name of the macro and the parameter
...
\Defgeneric {add-local-macro} {environment name expander}

This generic function is called by the compiler in order to add a
local macro to \textit{environment}.  The parameter \textit{name} is
