...
\subsubsection{Special operator information}
...
\textit{function-name} in scope in \textit{environment}.  The value of
...
verified that there is a local function named \textit{function-name} in
scope in \textit{environment}.  The function returns a new environment
augmented with the new information.  The original environment is not
affected.
...
\textit{optimize} parameter should be a complete and normalized
\texttt{optimize} specification, as returned by
\texttt{cleavir-policy:normalize-optimize}.
The value of the \textit{policy} parameter should be the policy
corresponding to the optimize specification, as computed by
\texttt{cleavir-policy:compute-policy}.
The original environment is not affected.
...
This generic function is called by the compiler in order to add an
inline expansion for a function. The compiler has already verified
that there is a local function or a global function named
\textit{function-name} in scope in \textit{environment}. The value
of the \textit{expansion} parameter is a \texttt{cleavir-ast:function-ast},
or \texttt{nil}, indicating that no expansion is available.
The original environment is not affected.
...
\textit{environment1}.  It is called as a result of top-level forms
being wrapped in \texttt{eval-when :compile-toplevel}. The
\textit{environment2} argument is only used for method dispatch.
...
This class represents a lexical variable.  It is a subclass of the
class \texttt{entry}

\Definitarg {:name}

This initarg is used to specify the name of the lexical variable.  The
value must be a symbol.  This initarg must be supplied.

\Defmethod {name} {(entry {\tt lexical-variable})}

Given an instance of the class \texttt{lexical-variable}, this method
returns the name of the lexical variable as supplied by the initarg
\texttt{:name}.
...
This initarg is used to specify the name of the special variable.  The
value must be a symbol.  This initarg must be supplied.
...
Given an instance of the class \texttt{special-variable}, this method
returns the name of the special variable as supplied by the initarg
\texttt{:name}.
...
This initarg is used to specify the name of the symbol macro.  The
value must be a symbol.  This initarg must be supplied.

\Definitarg {:expansion}

\Defmethod {name} {(entry {\tt symbol-macro})}

Given an instance of the class \texttt{symbol-macro}, this method
returns the name of the symbol macro as supplied by the initarg
\texttt{:name}.

\Defmethod {expansion} {(entry {\tt symbol-macro})}
...
This initarg is used to specify the name of the function.  The value
must be a function name.  This initarg must be supplied.

\Definitarg {:identity}

\Defmethod {name} {(entry {\tt function})}

Given an instance of the class \texttt{function}, this method returns
the name of the function as supplied by the initarg \texttt{:name}.

\Defmethod {identity} {(entry {\tt function})}
...
This initarg is used to specify the name of the macro.  The
value must be a symbol.  This initarg must be supplied.

\Definitarg {:expander}

\Defmethod {name} {(entry {\tt macro})}

Given an instance of the class \texttt{macro}, this method returns
the name of the macro as supplied by the initarg \texttt{:name}.

\Defmethod {expander} {(entry {\tt macro})}
...
This initarg is used to specify the name of the block.  The
value must be a symbol.  This initarg must be supplied.

\Definitarg {:identity}

\Defmethod {name} {(entry {\tt block})}

\Defmethod {identity} {(entry {\tt block})}
...
This initarg is used to specify the name of the tag.  The value must
be a go tag, i.e. a symbol or an integer.  This initarg must be
supplied.

\Definitarg {:identity}

\Defmethod {name} {(entry {\tt tag})}

\Defmethod {identity} {(entry {\tt tag})}
...
This initarg is used to specify the name of the variable.  The value
must be a symbol.  This initarg must be supplied.

\Definitarg {:type}

\Defmethod {name} {(entry {\tt variable-type})}

\Defmethod {type} {(entry {\tt variable-type})}
...
This initarg is used to specify the name of the function.  The
value must be a function name.  This initarg must be supplied.

\Definitarg {:type}

\Defmethod {name} {(entry {\tt function-type})}

\Defmethod {type} {(entry {\tt function-type})}
...
\Defmethod {name} {(entry {\tt variable-ignore})}

\Defmethod {ignore} {(entry {\tt variable-ignore})}
...
\Defmethod {name} {(entry {\tt function-ignore})}

\Defmethod {ignore} {(entry {\tt function-ignore})}
...
\Defmethod {name} {(entry {\tt variable-dynamic-extent})}
...
\Defmethod {name} {(entry {\tt function-dynamic-extent})}
...
\Defmethod {name} {(entry {\tt inline})}

\Defmethod {inline} {(entry {\tt inline})}

\subsection {Inline-expansion}

\Defclass {inline-expansion}
...
\Defmethod {name} {(entry {\tt inline-expansion})}

\Defmethod {ast} {(entry {\tt inline-expansion})}
...
\Defmethod {optimize} {(entry {\tt optimize})}

\Defmethod {policy} {(entry {\tt optimize})}
