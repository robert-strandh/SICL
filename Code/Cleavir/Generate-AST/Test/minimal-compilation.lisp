...
(defun test-flet ()
  ...
  (test '(flet ((fun (gsm3 &optional (x gsm3)) (+ x gsm3)))
	  (fun gsm3))
	468)
  ;; Test that a required parameter of the local function shadows the
  ;; global symbol macro in the &KEY part of the lambda list of
  ;; the local function, but not in the body of the FLET.
  (test '(flet ((fun (gsm3 &key (x gsm3)) (+ gsm3 x)))
	  (fun gsm3))
	468)
  ;; Test that a required parameter of the local function shadows the
  ...
  ;; lambda list of the local function.
  (test '(flet ((fun (&optional (gsm3 12) (x (1+ gsm3))) x))
	  (fun))
	13)
  ;; Test that an &OPTIONAL parameter of the local function shadows
  ...
  ;; the local function.
  (test '(flet ((fun (&optional (gsm3 12) &aux (x (1+ gsm3))) x))
	  (fun))
	13)
  ;; Test that an &OPTIONAL parameter of the local function shadows
  ;; the global symbol macro in body of the local function.
  (test '(flet ((fun (&optional (gsm3 12)) gsm3))
	  (fun))
	12)
