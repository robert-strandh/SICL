...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; Compile a FUNCALLABLE-SLOT-READ-AST

(defmethod compile-ast ((ast cleavir-ast:funcallable-slot-read-ast) context)
  (let ((temp1 (make-temp))
	(temp2 (make-temp)))
    (compile-ast
     (cleavir-ast:object-ast ast)
     (clone-context
      ...
      (list (compile-ast
	     ...
	     (clone-context
              ...
              :results (list temp2)
              :successors
              (list (make-instance 'cleavir-ir:funcallable-slot-read-instruction
                      :inputs (list temp1 temp2)
                      :outputs (results context)
                      :successors (successors context))))))))))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; Compile a FUNCALLABLE-SLOT-WRITE-AST

(defmethod compile-ast ((ast cleavir-ast:funcallable-slot-write-ast) context)
  (let ((temp1 (make-temp))
	(temp2 (make-temp))
	(temp3 (make-temp)))
    (compile-ast
	     ...
	     (clone-context
	      ...
	      (list (compile-ast
                     ...
                     (clone-context
                      ...
                      :results (list temp3)
                      :successors
                      (list (make-instance 'cleavir-ir:funcallable-slot-write-instruction
                              :inputs (list temp1 temp2 temp3)
                              :outputs '()
                              :successors (successors context)))))))))))))
