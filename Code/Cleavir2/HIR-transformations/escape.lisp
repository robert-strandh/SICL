...
;;; Return a list of pairs (e1 . e2), where e1 and e2 are
;;; ENTER-INSTRUCTIONs.  Each pair indicates that e1 may call e2.  If
;;; a function can escape, it could be called by anything. The graph
;;; does not include this fact.  ORIGINS is the result of
;;; compute-origins below.
...
;;; Run some analysis on the simple functions in the given
;;; initial-instruction.  It's top-down - if a simple function
;;; contains other simple functions, only the simple function highest
;;; up will be analyzed.
;;;
;;; So e.g. if L1 includes L2 and L3, and L2 includes L4, and other
;;; than L1 they're all simple, analyze-simples of L1 will call
;;; analyzer on only L2 and L3.
;;;
;;; FIXME: May analyze an insruction more than once if there are
;;; multiple encloses of one ENTER.  But this function is not used at
;;; the moment anyway.
...
;;; If the Boolean is T, the function does not have any sub-functions that escape.
    ...
    (cleavir-ir:map-instructions-arbitrary-order
     (lambda (i)
       (when (typep i 'call-instruction)
         (let ((origin '())
               (worklist (list (first (cleavir-ir:inputs i)))))
