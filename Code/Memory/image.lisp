...
(defun memory-unsigned (address size)
...
(defun memory-signed (address size)
  (let ((unsigned (memory-unsigned address size)))
    (if (>= unsigned (ash 1 (1- size)))
        (- unsigned (ash 1 size))
        unsigned)))

(defun (setf memory-unsigned) (value address size)
  (check-address-and-size address size)
  (multiple-value-bind (page page-index position)
      (split-address address)
...
(defun (setf memory-signed) (value address size)
  (let ((unsigned
          (if (minusp value)
              (+ value (ash 1 size))
              value)))
    (setf (memory-unsigned address size) unsigned)))
