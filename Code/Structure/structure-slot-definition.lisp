...
(defmethod mop:compute-default-initargs ((class structure-class))
...
(defmethod mop:direct-slot-definition-class ((class structure-class) &rest initargs)
...
(defmethod mop:effective-slot-definition-class ((class structure-class) &rest initargs)
  (declare (ignore initargs))
  (find-class 'structure-effective-slot-definition))

(defmethod mop:compute-effective-slot-definition :around ((class structure-class) name direct-slot-definitions)
...
(defmethod (setf mop:slot-value-using-class) :before (new-value (class structure-class) object (slot structure-effective-slot-definition))
...
(defmethod mop:slot-makunbound-using-class :before ((class structure-class) object (slot structure-effective-slot-definition))
  (when (and (structure-slot-definition-read-only slot)
             ;; As a special exception, allow unbound/uninitialized slots to
             ;; be initialized.
             (mop:slot-boundp-using-class class object slot))
