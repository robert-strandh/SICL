...
\vskip 0.25cm
Perhaps we would like to implement \texttt{reverse} in \commonlisp{}.
Here is a reasonable-looking implementation:
\vskip 0.25cm
\begin{verbatim}
(defun reverse (list)
  (loop with result = '()
...
\end{verbatim}
\vskip 0.25cm
But for this implementation to work, the \texttt{loop} macro must exist.
\vskip 0.25cm
And it is entirely possible that the macro expander for \texttt{loop}
needs \texttt{reverse}.
}
...
\vskip 0.25cm
\begin{itemize}
\item Write \texttt{reverse} in the implementation language of the
  core (C).
\item Use a simpler iteration construct, hoping its expansion does not
  require \texttt{reverse}.
\item Use \texttt{tagbody} and \texttt{go} to implement \texttt{reverse}.
\end{itemize}
\vskip 0.25cm
