...
We state several advantages in terms of bootstrapping, sandboxing, and
more.  We show an implementation where there is no performance penalty
associated with making the environment first class.  For performance
purposes, the essence of the implementation relies on the environment
containing \emph{cells} (an ordinary \texttt{cons} cell in our
implementation) holding bindings of names to functions and global
values that are likely to be heavily solicited at runtime.
\end{abstract}
