...
\begin{itemize}
\item a file translating compiler contains essentially only code, and
  code is generated from source statements and expressions as the
  implementation language makes available, whereas
\item a \commonlisp{} system also contains complex data structures,
  and these data structures are best created programmatically.
\end{itemize}
...
\item Create a mirror of the final executable as an octet vector in
  the memory of a host \commonlisp{} system.  This strategy is used by
  SBCL to create an initial ``core'' image.  It is also the strategy
  used by SICL.
\end{enumerate}
