;; divide by something that depends on the length of the list
(defun count-from-end-with-length-4-macro (x list length)
...
;;			  ((<= length 262144) (divide rest length 3))
			  (t (divide rest length 4)))))
	  (traverse list length))))
    count))

(defun reverse-count-4-macro (x list)
  (count-from-end-with-length-4-macro x list (length list)))
