...
Likke \allegro{}, \lispworks{} also provides the functionality for
...
\item The function \textit{function} is called for each local function
  ...
  in the CLtL2 protocol.  When \textit{kind} is
  \texttt{:macro}, then \textit{info} is the macro-expansion function;
  otherwise, \textit{info} is unspecified.
...
\item The function \textit{tag} is called for each tag in the
  ...
  is the keyword symbol \texttt{:tagg}, and \textit{info} is
  unspecified.
\end{itemize}
